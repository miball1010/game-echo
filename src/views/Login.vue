...
const { register, resetPassword, login, testLogin } = store
</script>

<template>
    <!-- 背景圖 -->
    <img :src="bg" alt="電玩背景" aria-hidden="true" class="fixed top-0 w-full h-full object-cover" />
    <div class="fixed top-0 w-full h-full bg-[#082530]/55 backdrop-blur-[5px]"></div>
        ...
        <transition name="slide-fade" mode="out-in">
            ...
            <div v-if="loginStatus.status === 'login'" key="login"
                ...
                <button @click="testLogin"
                    class="btn w-full mb-2 bg-[#EDAA5F] flex items-center justify-center gap-2 hover:opacity-90">
